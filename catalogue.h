...
//
struct Index {
  Index(ups_db_t *db_ = nullptr, Field *field_ = nullptr, bool enable_duplicates_ = false,
...
struct Table {
  Table(std::string name_)
      : name(std::move(name_)), ref_length(0), initial_autoinc_value(0), autoinc_value(0),
...
struct Database {
  ...
  Database(std::string name_)
      : name(std::move(name_)), env(nullptr), srv(nullptr), is_server_enabled(false),
